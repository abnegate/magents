...
Publishing a GitHub Release sets `package.version` from the tag, regenerates `Cargo.lock`, then pushes GHCR (`ghcr.io/abnegate/magents`), attaches musl/darwin binaries, and updates `abnegate/homebrew-tap` plus `abnegate/apt-repo`. Pushing a tag alone does not. If the release commit is the default-branch tip, CI also commits that version bump. Manual `package.version` edits still need `cargo update -p magents` so `--locked` CI does not drift.
